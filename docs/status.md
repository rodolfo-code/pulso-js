...
| ----------------- | -------------- | --------------------- | ----- |
| `health`          | não iniciado   | —                     | **Fase 1** — primeiro a migrar. Pequeno e independente; serve como spike pra validar o harness. |
...
| -------------------- | -------------- | ----- |
| `config`             | não iniciado   | `@Global` — `@nestjs/config` + ValidationPipe |
| `prisma`             | não iniciado   | `@Global` — `PrismaService` |
...
| `langfuse-client`    | não iniciado   | `@Global` — cliente HTTP Langfuse |
