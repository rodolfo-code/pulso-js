...
| `intelligence`   | migrado      | —     | **Fase 4** — `ComputeHealthScoreUseCase` (KPIs do Langfuse + heartbeat age + CB count → health score 0-100). 2 rotas (`GET /intelligence/agents/:slug/health-score`, `GET /intelligence/agents`). 3 services puros (`extractHierarchy`, `calculateKpis`, `computeHealthScore`) com 22 testes property-based. Migration `init_intelligence` (HealthScoreHistory, AgentSnapshot). Tech debt documentada em [tech-debt/fase-4-intelligence.md](tech-debt/fase-4-intelligence.md). Validado em Docker 2026-06-12. |
| `slos`           | migrado      | —     | **Fase 5** — `CreateSLOUseCase` + `EvaluateSLOUseCase` com roteamento pra CB metrics (`cb_open_duration_minutes`, `cb_open_count`, `cb_availability_pct`, com suporte a `:cb_name`). 4 rotas (`POST /slos`, `GET /slos`, `POST /slos/:id/evaluate`, `GET /slos/:id/evaluations`). `ParseUUIDPipe` nos params evitando 500 com ids malformados. 2 services puros: `cb-metrics` e `slo-evaluator` (operadores + extract metric + parse). Migration `init_slos` (SLODefinition, SLOEvaluation) + extensão de `ICircuitBreakerRepo` com `listHistory`. Tech debt em [tech-debt/fase-5-slos.md](tech-debt/fase-5-slos.md). Validado em Docker 2026-06-13. |
| `observations`   | migrado      | —     | **Fase 6** — `GetClientOverviewUseCase` (agrupa traces por userId, enriquece com agent do Postgres) + `GetConversationDetailUseCase` (session + traces enriquecidos com hierarchy). 4 rotas (`GET /observations/clients`, `GET /observations/clients/:clientId`, `GET /observations/conversations`, `GET /observations/conversations/:sessionId`). **Divergência consciente do legado**: removido `session.traces` (que vinha duplicado do Langfuse) — payload ~50% menor. Tech debt em [tech-debt/fase-6-observations.md](tech-debt/fase-6-observations.md). Validado em Docker 2026-06-13. |
...
| `domain`          | migrado      | Entities `Tenant`, `System`, `Agent` + VOs `AuditLogEntry`, `AgentStatus`, `CircuitBreakerState/Transition`, `HierarchyFields`, `KPIResult`, `HealthScore`, `HealthScoreRecord`, `AgentSnapshot`, `SLODefinition`, `SLOEvaluation`, `CBMetrics` + errors + services `taxonomy`, `status-engine`, `extract-hierarchy`, `kpi-calculator`, `health-score`, `cb-metrics`, `slo-evaluator`. |
| `repositories`    | migrado      | `@Global` `RepositoriesModule` — binds `IAuditLogRepo`, `IAgentRepo`, `ICircuitBreakerRepo` (com `listHistory`), `IHealthScoreRepo`, `ISnapshotRepo`, `ISLORepo` + mappers. |
