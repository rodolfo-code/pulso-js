...
| `domain`          | em andamento | Entities `Tenant`, `System`, `Agent` + VOs `AuditLogEntry`, `AgentStatus`, `CircuitBreakerState/Transition`, `HierarchyFields`, `KPIResult`, `HealthScore`, `HealthScoreRecord`, `AgentSnapshot` + errors + services `taxonomy`, `status-engine`, `extract-hierarchy`, `kpi-calculator`, `health-score`. Faltam `SLODefinition`, `SLOEvaluation`. |
| `repositories`    | em andamento | `@Global` `RepositoriesModule` — binds `IAuditLogRepo`, `IAgentRepo`, `ICircuitBreakerRepo`, `IHealthScoreRepo`, `ISnapshotRepo` + mappers. Falta `ISLORepo`. |
